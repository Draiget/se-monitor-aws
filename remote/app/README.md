...
- function_operate - Code for the operating application part that calls processing lambda.
- function_process - Code for the processing part of the application.
- events - Invocation events that you can use to invoke the function (locally, for API GW testing changes are required).
...
Application are provisioning by Terraform manifests presented in `infrastracture/` folder and require Lambda functions
to be packed before running Terraform. To do that, you should run .`/lambda_build.sh` script and it will pack everything
we need to according *zip* archives. 
...
for the AWS Lambda function(s) included to access AWS services. By default, these are scoped down to minimum required permissions. 
...
so that in the future you can just re-run `sam deploy` without parameters to deploy changes to your application.
...
The SAM CLI installs dependencies defined in `hello_world/requirements.txt`, creates a deployment package, 
and saves it in the `.aws-sam/build` folder.

Test a single function by invoking it directly with a test event. An event is a JSON document that represents 
the input that the function receives from the event source. Test events are included in the `events` folder in this project.

Run functions locally and invoke them with the `sam local invoke` command.

```bash
app$ sam local invoke SmProcessFunction --event events/process_find.json
```
...
```bash
app$ sam logs -n SmProcessFunction --stack-name app --tail
```
...
```bash
app$ pip install pytest pytest-mock --user
